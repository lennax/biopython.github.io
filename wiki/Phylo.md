...
[`matplotlib`](http://matplotlib.sourceforge.net/)` and either `[`PyGraphviz`](http://networkx.lanl.gov/pygraphviz/)` or `[`pydot`](http://dkbza.org/pydot.html)`.`
...
shown; these are clade names, but can be something else generated by the
'labeller' argument (which works the same as in draw\_graphviz). The
width of the text field used for drawing is 80 characters by default,
adjustable by keyword argument, and the height in character rows is
twice the number of terminals in the tree.
