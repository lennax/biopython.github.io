...
PYTHONPATH. Paths in this environment variable are listed in the same
manner as in the shell variable PATH. When PYTHONPATH is not set, or
when the file is not found there, the search continues in an
installation-dependent default path; on Unix, this is usually
.:/usr/local/lib/python.

It is important to note that this behavior differs from the default in
that regard that modules are not looked up in the current path or, more
precisely, modules are not searched in the list of directories given by
the variable sys.path, which is initialized from the directory
containing the input script, PYTHONPATH and the installation-dependent
default. The reason is that sys and sys.path are not imported into the
embedded Python environment. A user that relies on scripts being in the
same directory as a client program should add "." (the same applies to
any other specific directory) to the PYTHONPATH environment variable.
...
lifetime of plug-in factories. Upon the destruction of a node, ess and
dens will try to decrement their reference count (thus effectively
freeing up the resources allocated by the Python interpreter) and the
valid Python interpreter instance must exist. Failure to ensure the
proper destruction order will result in undefined behavior.
